"""
Aplica la configuración TR-069 a todos los routers listados en pendientes.txt
(generado por verificar_estado_acs.py). Va uno por uno, con pausa entre cada
uno, y nunca corta el proceso si alguno falla -- sigue con el resto y deja
un reporte final.
"""
...
def main():
    ...
    total = len(urls)
    print(f"Configurando {total} router(es) desde {INPUT_FILE}...\n")
    ...
    if error:
        print("\nRouters con error (revisar):")
        for r in error:
            print(f"  {r['url']}: {r['detalle']}")
